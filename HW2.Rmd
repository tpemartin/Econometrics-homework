...
```
加入公會者，受到工會保護，工時應該較短，同時公會具有勞動供給的談判力，故公會成員薪資應該較高。因此我們會擔心會低估係數。表示事實上薪水高我們應該很樂意工作，但加上了工會保護，我們反而可能工時更短。
```
...
```
簡單回歸不顯著，複回歸顯著。係數表示增加薪水對工時的誘因影響，又在複回歸中係數比簡單回歸係數更大，表示簡單回歸中，我們低估了薪水對工時的誘惑力，其實工時變短可能是因為其它因素導致不會(想)工作那麼多。
```
...
```
在控制變數下，表示有一群性別相同、教育相同、公會偏好相同。
舉例： 一群加入公會且都大學畢業的男生，可能工作地點在不同的區域(北部與南部)，要求的工時長度不一(因為法律管束較少，越接近行政區或大都會，越遵守法規)
```
