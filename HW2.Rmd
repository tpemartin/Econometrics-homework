...
```
1.產業別：產業的技術經濟特點影響工資水平，資本密集型產業工資較高，反之較低，也就是隨著產業別的不同薪資會有所差距，但如果假設在同樣的薪資水準下，勞工不會因為從事資本密集型產業就想多工作或少工作，也就是不管從事任何產業，在同樣的薪資下工作時數並不會受到影響，因此產業別可能是合理的工具變數。
2.居住地：都市的消費水準比鄉下高，所以通常都市的薪資也會比鄉下高，也就是隨著居住地的不同薪資會有所差距，但如果假設在同樣的薪資水準下，勞工不會因為住在都市就想多工作或少工作，也就是不管在都市或鄉下，在同樣的薪資下工作時數並不會受到影響，因此居住地可能是合理的工具變數。
```
...
```{r 比較四種結果}
#建立model
model2%>%update.formula(~.+ IND ) ->model3
model2%>%update.formula(~.+ IND+ SMSA ) ->model4
#跑OLS
tsls1iv1<-lm(model3,labordata)
tsls1iv2<-lm(model4,labordata)
#使用vcovHC函數來計算HC1型的異質變異（即橫斷面資料下的線性迴歸模型
coeftest(tsls1iv1, vcov. = vcovHC, type="HC1") -> tsls1iv_coeftest
coeftest(tsls1iv2, vcov. = vcovHC, type="HC1") -> tsls2iv_coeftest
#比較結果
...
```
職位：以航空業為例，飛行員比較多是男性，而機組人員比較多是女性，所以性別和職位有一定的相關性，而且飛行員的薪水本來就高於機組人員，所以職位也會影響到薪水。如果單純比較男性跟女性的的薪資，可能只是在比較不同職位間的薪資差距。
```