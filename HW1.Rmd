...
```
我們認為會造成選擇性偏誤的兩個變數，分別為exper和smsa。exper是因為，通常工作年數愈長的人，受教育年數愈短；這群人可能很早出社會，且從事低報酬工作。smsa為較繁榮區域，可能住在smsa的人就是有比較好的薪資，同時更有誘因接受長年數教育。所以比較教育年數的差距，可能只是在比較exper的差(是否從事低報酬工作)，或是是否住在smsa (繁榮地區)。
比對所有變數與educ的相關係數可知，exper、numdep、smsa、profserv、profocc都和educ有超過20%的相關性，其中又以exper的-29.9%、smsa的22.4%和profocc的49.6%為最高。此題是討論哪些變數可能造成選擇性偏誤，在此問題中，職業是否為教授應非造成選擇性偏誤的原因（目的是比較教育效果，因此只探討職業是否為教授邏輯很怪）；profocc與educ相關係數高，也許是因教授通常要求學歷，教育年數較長。
```
