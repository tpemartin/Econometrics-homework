...
```{r}
summary(lm(log(wage1$wage)~wage1$educ))
...
其中，推測性別和膚色這兩個類別變數對於教育年限本身就有解釋能力，而不控制這兩個變數將可能導致估計結果的偏誤
...
教育年限於12年以上及16年以下的資料，有可能是大學輟學生，未完成大學學業
...
wage1 %>%
  group_by(educ<=12 , 13<=educ&educ<=15, 16<=educ) %>%
  summarise(mean(wage,na.rm=T), length(wage))   ####################### 蠢到不行的方法##################

```
由以上的表可以看到，修業年限為16年以上的的人共有99筆資料，平均薪資最高，為8.948；而修業年限介於13-15年間的人，推測未完成大學學業有113筆資料，其平均薪資為6.03，而修業年限小於等於12年的人有314人，平均薪資僅4.886
...
則選擇上大學將有 99/(99+113) 的機率可以完成學業，使薪資期望值上升至8.95，而有113/(99+113)的機率無法完成學業，薪資僅上升至6.03
則選擇上大學的預期薪資好處為  8.947980 * 99/(99+113) + 6.03 * 113/(99+113) - 4.885701 = 2.506941 per hour

```{r}
8.947980 * 99/(99+113) + 6.03 * 113/(99+113) - 4.885701
