...
```
educ與wage的相關度如程式結果所示。由圖可發現，wage的資料是左偏的。為了讓資料符合對稱(鐘形分配)，改考慮wage的變化率，即log(wage)與educ的相關度。而結果顯示，log(wage)確實較符合鐘形分配。另外，因為educ是教育年限，取變化率沒有經濟意義，且原資料的偏度接近0，故不對educ做更動。
```
...
```
我們認為造成選擇性偏誤的變數，分別為exper和smsa。exper是因為，通常（給定一定年紀下）工作年數愈長的人，受教育年數愈短；這群人可能很早出社會，且從事低報酬工作。而smsa為較繁榮區域，或許能住smsa的人薪資本來就比較高，同時有誘因接受長年數教育。所以比較教育年數的差距，可能只是在比exper的差(是否從事低報酬工作)或是不是住在smsa (繁榮地區)。
比對所有變數與educ的相關係數可知，exper、numdep、smsa、profserv、profocc和educ的相關性都超過20%，其中又以exper的-29.9%、smsa的22.4%和profocc的49.6%為最高。此題是討論哪些變數可能造成選擇性偏誤，在此問題中，職業是否為教授應非造成選擇性偏誤的原因（目的是比較教育效果，因此只探討職業是否為教授邏輯很怪）；profocc與educ相關係數高，也許是因教授通常要求學歷，教育年數較長。
```
