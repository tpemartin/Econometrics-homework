...
　     construc,ndurman,trcommpu,trade,services,profserv,profocc,clerocc,servocc代表職業，分別代表建造業(construc=1)、非耐久財製造業(ndurman=1)、翻譯出版業(trcommpu=1)、
　     貿易業(trade=1)、加工業(service=1)、精工業(profserv=1)、教授(profocc=1)、牧師(clerocc=1)與服務業(servocc=1)
　     lwage代表wage取Log的數值
　     expersq代表exper的平方項
　     tenursq代表tenure的平方項
2. educ與wage的相關度如程式結果所示。由圖可發現，wage的資料是左偏的。為了讓資料符合對稱(鐘形分配)，故改考慮wage的變化率，即log(wage)與educ的相關度。而結果顯示，log(wage)確實較符合鐘形分配。另外，因為educ是教育年限，取變化率沒有經濟意義，且原資料的偏度接近0，故不對educ做更動。
```
...
```
我們認為會造成選擇性偏誤的兩個變數，分別為exper和smsa。exper是因為，通常能有愈多工作年數的人，可能受過的教育年數愈短，這群人可能很早就出社會工作，並且從事較低報酬的工作。smsa為較繁榮的人口集中區域，可能住在smsa的人，就是有比較好的薪資，和教育年數沒有關係，也同時更有誘因接受更常年數的教育。所以比較教育年數的差距，可能只是在比較exper的差(是否從事低報酬工作)，或是是否住在smsa。
