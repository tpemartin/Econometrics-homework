...
2. 本資料一共有24個變數。526筆觀測值
  ...
  7. married                  1，已婚；0，未婚
  8. numdep                   孩子個數
  9. smsa                     =1 if live in SMSA (如果住在都會區)
 10. northcen                 =1 if live in north central U.S （是否住在美國北部）
 11. south                    =1 if live in southern region （是否住在美國南部）
 12. west                     =1 if live in western region（是否住在美西）
 13. construc                 =1 if work in construc. indus.（是否從事營建業）
 14. ndurman                  =1 if in nondur. manuf. indus.（是否從事製造業？)
 15. trcommpuut                 =1 if in trans, commun, pub （是否從事運輸業、通訊產業、公共事業（類政府部門）
 16. trade                    =1 if in wholesale or retail（是否從事批發或零售等服務業）
 17. services                 =1 if in services indus.（是否從事服務業）
 18. profserv                 =1 if in prof. serv. indus.（是否從事專業服務業）（也許是會計師、顧問等等）
 19. profocc                  =1 if in profess. occupation（是否擔任專業職）
 20. clerocc                  =1 if in clerical occupation（是否擔任行政職）
 21. servocc                  =1 if in service occupation（是否擔任服務職）
 22. lwage                    log(wage)
 23. expersq                  exper^2
 24. tenursq                  tenure^2
```
...
```
註：針對類別資料的0/1，不是數值資料，而是類別資料的表達。那取log是沒有意義的。資料型態的問題也需要特別留意。
...
```
答案更正方向：在探究selection bias，需要去考量他和educ（X 主要解釋變數）和lwage（Y）的相關係數。不然就算他跟educ有關，跟lwage無關，那你加入他又有什麼用。這是觀念問題，操作上也是兩個相關係數都要表達。
```
